<!DOCTYPE html>
<html>
  <head>
    <title>Mickey Resume</title>

    <?php require "lib/inc/headData.php"; ?>

  </head>
  <body>

    <?php require "lib/inc/header.php"; ?>

    <div class="pageWrapper">

      <div id="downloadPDFButton"><a href="lib/inc/MickeyHernandezResume.pdf">Download PDF</a></div>

      <div class="resumePaper">
        <h2>Mickey Hernandez</h2>
        <ul id="noList">
          <li>(704)-343-1766</li>
          <li>mickeyh1998@gmail.com</li>
        </ul>

        <h2>Profile</h2>
        <p>Highly motivated, aspiring Front End Developer that excels in responsive, creative, and semantic web development. Passionate about CSS, user-friendly design, and intuitive UIs.</p>

        <h2>Technical Skills</h2>
        <ul id="twoColumns">
          <li>HTML 5</li>
          <li>CSS 3</li>
          <li>JavaScript</li>
          <li>jQuery</li>
          <li>React.js</li>
          <li>PHP 7</li>
          <li>Responsive Design</li>
          <li>Git</li>
        </ul>

        <h2>Experience</h2>
        <h3 class="halfDiv">RoadToHire / Student</h3>
        <p class="halfDiv">June 2017 - Current</p>
        <ul>
          <li>Learned several programming languages</li>
          <li>Applied tech skills to various projects</li>
          <li>Learned industry related soft skills</li>
        </ul>

        <h3 class="halfDiv">QuikTrip / Store Clerk</h3>
        <p class="halfDiv">February  2017 - June 2017</p>
        <ul>
          <li>Learned about convenience store business</li>
          <li>Worked in a team to provide excellent customer service</li>
          <li>Practiced time budgeting</li>
        </ul>

        <h3 class="halfDiv">Jack in the Box / Shift Manager</h3>
        <p class="halfDiv">May  2016 - February  2017</p>
        <ul>
          <li>Managed and operated all aspects of restaurant</li>
          <li>Oversaw training of all new trainees</li>
          <li>Resolved any customer complaints</li>
        </ul>

        <h3 class="halfDiv">Jack in the Box / Team Member</h3>
        <p class="halfDiv">December  2015 - May 2016</p>
        <ul>
          <li>Learned to work in a team</li>
          <li>Provided excellent customer service</li>
          <li>Promoted after 6 months</li>
        </ul>

        <h2>Education</h2>
        <p>Graduated 2016</p>
        <p>High School Diploma - Renaissance School of Technology @ Olympic High School</p>


      </div>
    </div>

    <?php require "lib/inc/footer.php"; ?>

  </body>
</html>
